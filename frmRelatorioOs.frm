...
Begin VB.Form frmRelatorioOs 
   ...
   Caption         =   "Relatório de Vendas por Clientes"
   ClientHeight    =   7635
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   12630
...
If rs.RecordCount = 0 Then
   MsgBox "Não foi encontrado nenhuma OS neste período.", vbInformation
   rs.Close
   Exit Sub
End If
...
End With
    ...
    If Text3.Text = "" Then
        ...
        OSbaixada = rs.RecordCount
        StatusBar1.Panels(3).Text = "O.S. não baixada: " & OSbaixada
        rs.Close
    Else
    ...
    End Select
         ...
         StatusBar1.Panels(3).Text = "Não baixada (" & OSNaoBaixada & ") "
...
With Printer
   ...
   While Not rs.EOF
      ...
      .FontSize = 9
      .FontBold = False
      .CurrentX = 11350 - .TextWidth("Página: " & nPagina & "/" & totalPagina)
      .CurrentY = 2250
      Printer.Print "Página: " & nPagina & "/" & totalPagina
...
With grdRelatorioOs
    .FormatString = "^ |^Nº OS|<Nome|^Data|>Valor|^Baixa|Atendente"
...
With Printer
   .CurrentY = 800
   .CurrentX = 11450 - .TextWidth("RELATÓRIO DE OS")
   Printer.Print "RELATÓRIO DE OS"
   ...
   If Option1(0).Value = True Then
      Printer.Print "OS baixada no período de " & Text1(0).Text & " à " & Text1(1).Text
   ElseIf Option1(1).Value = True Then
      Printer.Print "OS não baixada no período de " & Text1(0).Text & " à " & Text1(1).Text
   Else
      Printer.Print "OS emitida no período de " & Text1(0).Text & " à " & Text1(1).Text
   End If
   ...
   Printer.Line (500, 2500)-(11450, 2500) ' cabeça1
   Printer.Line (500, 2900)-(11450, 2900) ' cabeça2
   Printer.Line (500, 16200)-(11450, 16200) ' rodapé
End With
